...
router.get('/', async (req, res) => {
  try {
    ...
    // ✅ Pass userEmail to the template
    res.render('home', { 
      concerts, 
...
router.post('/login', [validateEmail, validatePassword], async (req, res) => {
  ...
  try {
    ...
    if (!user) return res.render('login', { message: 'Incorrect Email Address.', errors: [] });

    const isPasswordValid = await bcrypt.compare(password, user.password);
    if (!isPasswordValid) return res.render('login', { message: 'Incorrect password.', errors: [] });
...
router.post('/signup', async (req, res) => {
  ...
  try {
    ...
    if (existingUser) return res.render('signup', { message: 'Email already taken', error: null });
